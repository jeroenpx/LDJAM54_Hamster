...
class QuickAddDropdown : AdvancedDropdown
{
    public QuickAddDropdown(AdvancedDropdownState state) : base(state)
    {
    }
...
public class AdvancedDropdownTestWindow : EditorWindow
{
    void OnGUI()
    {
        ...
        {
            var dropdown = new QuickAddDropdown(new AdvancedDropdownState());
            dropdown.Show(rect);
        }
    }
}